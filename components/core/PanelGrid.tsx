...
import { ChatPanel } from "../chat-panel/ChatPanel"; // 標準版に変更
...
export function PanelGrid() {
  ...
  const activePanels = panels
    .filter((panel) => activePanelIds.includes(panel.id))
  ...
  const isSpecialLayout =
    activePanels.length === 5 || activePanels.length === 6;
  const specialGridClass =
    activePanels.length === 5
      ...
      {activePanels.length === 0 ? (
        ...
        <div
          className={cn(
            ...
            isSpecialLayout
              ? specialGridClass
              : getGridClassName(activePanels.length)
          )}>
